...
static timespec_t clock_realtime;	/* system time starting from 0:00 at power on */
...
static timespec_t clock_monotonic;	/* system time starting from 0:00 at power on */
...
{
	clock_realtime.tv_sec = clock_realtime.tv_nsec = 0;
	clock_monotonic.tv_sec = clock_realtime.tv_nsec = 0;
	alarm_handler = NULL;
...
void arch_timer_set(timespec_t *time, void *alarm_func)
{
	...
	time_add(&clock_realtime, &last_load);
